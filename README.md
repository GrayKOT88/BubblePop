...
Чтобы наглядно продемонстрировать разницу, в проекте представлены две версии кода:

| Папка | Описание | Назначение |
| :--- | :--- | :--- |
| **`Assets/_Project/Old Scripts/`** | Монолитный код "как было". | Содержит всю логику игры в одном скрипте. Демонстрирует проблемы "спагетти-кода": жесткая связанность, сложность добавления новых функций и тестирования. |
| **`Assets/_Project/New Scripts/`** | Рефакторинг "как стало". | Код разбит на независимые компоненты с четкими обязанностями. Применены паттерны проектирования (MVC, Factory, Observer и др.), что делает код гибким, расширяемым и тестируемым. |

💡 **Как использовать:** Чтобы увидеть эволюцию кода, достаточно сравнить скрипты в папках `Old Scripts` и `New Scripts` с одинаковыми названиями (например, `GameManager.cs`).
